...
class PostgresCodeGraphStore:
        ...
        try:
            with self._transaction() as cursor:
                ...
                return self._activate(
                    cursor, domain_id, session, record, rows, header, now
                )
    ...
    def _activate(
        self, cursor, domain_id, session, record, rows, header, now
    ) -> dict[str, object]:
        snapshot_id = record["snapshot_id"]
        markdown_revision = f"generation:{record['captured_markdown_generation']}"
