...
def test_report_is_deterministic_atomic_and_sanitized(tmp_path):
    ...
    evidence = {"decision": "blocked",
                "blocker": "Bearer SECRET_SENTINEL https://x.invalid/a /private/path",
...
def test_sanitizer_redacts_auth_dsn_and_non_json_values(tmp_path):
    ...
    class SecretError(Exception):
        def __str__(self):
            return "SECRET_SENTINEL /private/error"

    class Unsupported:
        def __repr__(self):
            return "SECRET_SENTINEL /private/object"
...
def test_cli_argument_validation_and_exit_mapping(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "build_evidence",
        lambda *args, **kwargs: {
            "decision": "implement", "public_registry_contains_tool": False,
        },
    )
...
def test_cli_registry_probe_flows_false_and_true_without_mutation(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: observed.append(
            kwargs["public_registry_contains_tool"]
        ) or {"decision": "blocked"},
    )
...
def test_report_summarizes_sampling_quality_failures_calls_and_registry():
    ...
    rendered = render_markdown({
        "decision": "do_not_implement", "model": "test", "sampling": {
            ...
            "total_attempts": 3, "included_pairs": 2, "excluded_pairs": 1,
            "per_case": {"a": {
                "total_attempts": 3, "included_pairs": 2, "excluded_pairs": 1,
        ...
        "quality": {
            "aggregate_lower_bound": -0.1, "scenario_lower_bounds": {"a": -0.2},
        ...
        "aggregates": {"per_case": {"a": {
            "candidate_success_rate": 0.75, "baseline_success_rate": 0.5,
        }}},
        ...
        "tool_calls": {"included_pair_candidate_mean": 2.0, "included_pair_baseline_mean": 4.0,
                       "included_pair_mean_difference": -2.0,
                       "excluded_attempt_calls": {"candidate_total": 1}},
        "public_registry_contains_tool": False,
    })

    for heading in (
        "## Sampling", "## Quality", "## Failure counts",
        "## Secondary tool calls", "## Registry state",
    ):
    ...
    for value in ("Total attempts: 3", "Included pairs: 2", "Excluded pairs: 1",
                  ...
                  "Decision gates",
                  'a: total 3; included 2; excluded 1;'
                  ' reasons {"failed_transport": 1}'):
...
def test_cli_capability_preflight_blocks_without_starting_sampling(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        cli,
        "build_evidence",
        lambda *args, **kwargs: called.append(kwargs) or {"decision": "blocked"},
    )
...
def test_tool_calling_preflight_does_not_cap_reasoning_budget():
    ...
    client = Client()
    assert cli._tool_calling_preflight(client, "model") == {
        "available": True, "status": "supported",
    }
...
def test_tool_calling_preflight_requires_complete_tool_envelope():
    ...
    assert cli._tool_calling_preflight(Client(malformed), "model") == {
        "available": False, "status": "failed_response",
    }
    valid = {
        "id": "call-1", "type": "function",
        "function": {"name": "preflight", "arguments": "{}"},
    }
    assert cli._tool_calling_preflight(Client(valid), "model") == {
        "available": True, "status": "supported",
    }
    nonempty = {
        "id": "call-1", "type": "function",
        "function": {"name": "preflight", "arguments": '{"extra": 1}'},
    }
    assert cli._tool_calling_preflight(Client(nonempty), "model") == {
        "available": False, "status": "failed_response",
    }
...
def test_http_500_is_safe_in_preflight_and_raised_by_sampling_post(tmp_path, monkeypatch):
    ...
    class FailedResponse:
        ...
        def json(self):
            raise AssertionError("JSON must not be read after HTTP 500")
    ...
    broken = type(
        "Broken", (), {"post": lambda *_args, **_kwargs: FailedResponse()}
    )()
    assert cli._tool_calling_preflight(broken, "model") == {
        "available": False, "status": "failed_transport",
    }
    monkeypatch.setenv("IWIKI_LLM_BASE_URL", "https://example.invalid/v1")
    monkeypatch.setenv("IWIKI_LLM_KEY", "key")
    monkeypatch.setenv("IWIKI_CHAT_MODEL", "model")
