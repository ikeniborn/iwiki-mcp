...
def _any_post(*_):
    """Stand in for the HTTP post object the stubbed arms never call."""
    return object()
...
def _agent_run(case, *, arm="baseline", status="ok", graph_state=None, missing=(), trace=(),
    ...
    if graph_state is None:
        graph_state = (
            "revision_changed" if case.id == "revision-mismatch"
            else case.expected_graph_state
        )
    ...
    if not tool_names:
        tool_names = (
            ...
            if arm == "baseline"
            else ("wiki_unified_search",)
        )
    if not trace:
        trace = tuple(
            {"name": name, "call_id": f"{arm}-{name}"} for name in tool_names
        )
    graph_correct = graph_state == case.expected_graph_state
    return AgentRun(
        ...
        graph_correct, status == "ok" and graph_correct and not missing,
        status, 2, [] if messages is None else messages, tool_names,
    )


def _complete_protocol_evidence():
    from eval.unified_search.runner import (
        BOOTSTRAP_SAMPLES, BOOTSTRAP_SEED, MAX_ATTEMPTS,
        NON_INFERIORITY_MARGIN, REQUIRED_PAIRS, _aggregate,
        _expected_graph_state, _expected_metadata, _failure_counts,
        _first_three_unique_seeds, _quality, _tool_calls, raw_parity,
    )
    ...
    for case_id in ids:
        for attempt in range(1, REQUIRED_PAIRS + 1):
            ...
            for arm in order:
                ...
                actual = tuple(facts)
                missing = [
                    item for index, item in enumerate(expected)
                    if index >= len(actual) or actual[index] != item
                ]
                extra = [
                    item for index, item in enumerate(actual)
                    if index >= len(expected) or expected[index] != item
                ]
                graph = _expected_graph_state(case)
                success = not missing and not extra
                names = (
                    ["wiki_unified_search"] if arm == "candidate"
                    else ["wiki_search", "wiki_code_search", "wiki_code_context"]
                )
                pair.append({
                    "case_id": case_id, "attempt": attempt, "run": attempt,
                    "arm": arm, "status": "ok", "success": success,
                    "correctness": success,
                    "client_visible_calls": 1 if arm == "candidate" else 3,
                    "result": {"fact_ids": facts, "graph_state": graph},
                    "missing_fact_ids": missing, "extra_fact_ids": extra,
                    "graph_state_correct": True,
                    "observed_context_seeds": _first_three_unique_seeds(
                        case.code.as_dict()
                    ),
                    "seed_mistakes": 0, "omitted_required_context_calls": 0,
                    "stale_claim_errors": 0, "missing_claim_errors": 0,
                    "revision_changed_claim_errors": 0,
                    "stale_missing_revision_claim_errors": 0,
                    "required_fact_loss": len(missing),
                    "tool_trace": [
                        {"name": name, "call_id": f"{arm}-{name}"}
                        for name in names
                    ],
                    "prompt_hash": "prompt", "environment_hash": "environment",
                    "shared_messages": [
                        {"role": "user", "content": "shared"}
                    ],
                    "tool_schema_hash": f"schema-{arm}",
                    "declared_tool_names": names,
                    "included": True, "exclusion_reason": None,
                })
            attempts.append({
                "case_id": case_id, "attempt": attempt, "arm_order": order,
                "included": True, "exclusion_reason": None, "rows": pair,
            })
            rows.extend(pair)
    evidence = {
        "model": "test", "transport_configured": True,
        "tool_calling_available": True,
        "preflight": {"available": True, "status": "supported"},
        "public_registry_contains_tool": False,
        "protocol": {
            "expected_case_ids": ids, "required_pairs": REQUIRED_PAIRS,
        ...
        "raw_parity": sorted(
            (raw_parity(case) for case in FIXED_CASES),
            key=lambda row: row["case_id"],
        ),
        "sampling": {
            "required_pairs": REQUIRED_PAIRS, "max_attempts": MAX_ATTEMPTS,
            "complete": True, "attempt_cap_exhausted": False,
            "per_case_included_pairs": {
                case_id: REQUIRED_PAIRS for case_id in ids
            },
        },
        "attempts": attempts, "run_results": rows,
        "attempt_counts": {
            "total_attempts": len(attempts), "included_pairs": len(attempts),
            "excluded_pairs": 0,
            "per_case": {
                case_id: {
                    "total_attempts": REQUIRED_PAIRS,
                    "included_pairs": REQUIRED_PAIRS,
                    "excluded_pairs": 0, "exclusion_reasons": {},
                }
                for case_id in ids
            },
            "exclusion_reasons": {},
...
def test_decision_gates_use_quality_bounds_not_legacy_counts_or_calls():
    ...
    for key, value in (("transport_configured", False), ("model", "")):
        ...
        assert decide(evidence)["decision"] == "do_not_implement"
    blocked_preflight = {
        **good, "preflight": {"available": False, "status": "failed_response"},
    }
    assert decide(blocked_preflight)["decision"] == "do_not_implement"
...
def test_raw_parity_exposes_independent_context_booleans(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda **_: {
            **runner.ideal_specialized_result(case),
            "context": {"wiki_pages": ["bad"]},
        },
    )
...
def test_run_metrics_capture_seed_and_state_claim_categories():
    ...
    ready = FIXED_CASES[0]
    row = _run_metrics(
        _agent_run(ready, trace=({"name": "wiki_search", "call_id": "x"},)),
        ready,
        observed_seeds=["wrong"],
    )
    ...
    missing_row = _run_metrics(_agent_run(missing, graph_state="ready"), missing)
    assert missing_row["missing_claim_errors"] == 1
    revision_row = _run_metrics(
        _agent_run(revision, graph_state="wrong"), revision
    )
    assert revision_row["revision_changed_claim_errors"] == 1
...
def test_workflow_revision_ready_claim_cannot_pass(monkeypatch):
    ...
    monkeypatch.setattr(
        runner,
        "run_agent_case",
        lambda case, arm, model, post: _agent_run(
            case, arm=arm, graph_state="ready"
        ),
    )
    evidence = runner.run_workflow([revision], runs=3, model="test", post_factory=_any_post)
...
def test_transport_failure_excludes_pair_and_exhaustion_is_not_blocked(monkeypatch):
    ...
    monkeypatch.setattr(
        runner,
        "run_agent_case",
        lambda _case, arm, *_: _agent_run(
            case, arm=arm, status="failed_transport"
        ),
    )
    evidence = runner.run_workflow([case], runs=1, model="test", post_factory=_any_post)
...
def test_workflow_captures_only_baseline_context_seeds():
    ...
    def factory(_case, arm, _run):
        ...
        if arm == "baseline":
            return Post([
                response(tool_calls=[call("wiki_search", {"query": "private"})]),
                response(
                    tool_calls=[call("wiki_code_search", {"query": "private"})]
                ),
                response(
                    tool_calls=[call("wiki_code_context", {"seeds": ["wrong"]})]
                ),
                final,
            ])
        return Post([
            response(tool_calls=[call("wiki_unified_search", {"query": "private"})]),
            final,
        ])
...
def test_candidate_seed_spy_catches_wrong_composer_input(monkeypatch):
    ...
    seed_row = runner._run_metrics(
        _agent_run(case, arm="candidate"), case, ["wrong"]
    )
    assert seed_row["seed_mistakes"] == 1


def _aggregate_row(case_id, arm, calls):
    return {
        "case_id": case_id, "arm": arm, "correctness": True,
        "success": True, "client_visible_calls": calls,
    }
...
def test_aggregate_retains_secondary_call_metrics_without_decision_gate():
    ...
    other = next(
        ...
        if case.id == "sqlite-postgres-hosted-labels"
    )
    rows = [
        _aggregate_row(coordinated.id, "candidate", 4),
        _aggregate_row(coordinated.id, "baseline", 3),
        _aggregate_row(other.id, "candidate", 1),
        _aggregate_row(other.id, "baseline", 7),
    ]
...
def test_workflow_aggregates_every_run_not_best_of(monkeypatch):
    ...
    def fake_run(case, arm, model, post):
        ...
        return AgentRun(
            arm, case.id, model, "env", "prompt", "schema", (),
            {"fact_ids": facts, "graph_state": case.expected_graph_state},
            case.expected_fact_ids, case.expected_graph_state,
            () if success else case.expected_fact_ids, (), True, success,
            "ok", 1 if arm == "candidate" else 3, [], (),
        )

    monkeypatch.setattr(runner, "run_agent_case", fake_run)
    evidence = runner.run_workflow([case], runs=3, model="test", post_factory=_any_post)
...
def test_workflow_retries_only_transport_pairs_and_keeps_every_attempt(monkeypatch):
    ...
    assert evidence["attempt_counts"] == {
        "total_attempts": 2, "included_pairs": 1, "excluded_pairs": 1,
        "per_case": {case.id: {
            "total_attempts": 2, "included_pairs": 1, "excluded_pairs": 1,
...
def test_decide_uses_pairwise_bounds_and_preflight_only_blocks():
    ...
    zero_bounds = {
        **complete,
        "quality": {
            "aggregate_lower_bound": 0.0, "scenario_lower_bounds": {"a": 0.0},
        },
    }
    assert decide(zero_bounds)["decision"] == "do_not_implement"
...
def test_decide_requires_unchanged_fixed_registration_protocol():
    ...
    one_case_only = {
        **evidence,
        "sampling": {
            **evidence["sampling"], "per_case_included_pairs": {ids[0]: 20},
        },
    }
    assert decide(one_case_only)["decision"] == "do_not_implement"
    narrower_margin = {
        **evidence,
        "quality": {**evidence["quality"], "non_inferiority_margin": 0.14},
    }
    assert decide(narrower_margin)["decision"] == "do_not_implement"
...
def test_decide_rejects_forged_protocol_without_retained_attempts():
    ...
    evidence.update({
        ...
        "attempts": [],
        "attempt_counts": {
            "total_attempts": 0, "included_pairs": 0, "excluded_pairs": 0,
            "per_case": {}, "exclusion_reasons": {},
        },
        "run_results": [],
    })
    assert decide(evidence)["decision"] == "do_not_implement"
...
def test_scheduler_collects_exactly_twenty_pairs_for_every_fixed_case(monkeypatch):
    ...
    monkeypatch.setattr(
        runner,
        "run_agent_case",
        lambda case, arm, *_: _agent_run(case, arm=arm),
    )
    monkeypatch.setattr(
        runner,
        "_quality",
        lambda *_: {
            "aggregate_lower_bound": 1.0, "scenario_lower_bounds": {},
            "non_inferiority_margin": 0.15, "bootstrap_samples": 50_000,
            "bootstrap_seed": 20260826,
        },
    )
    evidence = runner.run_workflow(FIXED_CASES, runs=20, model="test", post_factory=_any_post)
    ...
    assert all(
        ...
        for counts in evidence["attempt_counts"]["per_case"].values()
    )
...
def test_build_evidence_reaches_implement_from_full_scheduler(monkeypatch):
    ...
    monkeypatch.setattr(runner, "_expected_metadata", lambda _case, arm, _model: {
        "environment_hash": "env", "prompt_hash": "prompt",
        "tool_schema_hash": "schema", "shared_messages": [],
        "declared_tool_names": (
            ["wiki_search", "wiki_code_search", "wiki_code_context"]
            if arm == "baseline" else ["wiki_unified_search"]
        ),
    })
    monkeypatch.setattr(
        runner, "paired_lower_bound", lambda *_args, **_kwargs: 1.0
    )
    evidence = runner.build_evidence(
        FIXED_CASES, runs=20, model="test", transport_configured=True,
        tool_calling_available=True,
        preflight={"available": True, "status": "supported"},
        post_factory=_any_post, public_registry_contains_tool=False,
    )
...
def test_runner_exception_creates_complete_included_failure_rows(monkeypatch):
    ...
    monkeypatch.setattr(
        runner,
        "run_agent_case",
        lambda *_: (_ for _ in ()).throw(RuntimeError("runner")),
    )
    evidence = runner.run_workflow(
        [case], runs=1, model="test", post_factory=_any_post
    )
    assert evidence["attempts"][0]["included"] is True
    assert all(
        row["status"] == "failed_runner" and not row["success"]
        for row in evidence["attempts"][0]["rows"]
    )
