...
def test_wiki_code_index_keeps_sqlite_result_flat(seed_binding, monkeypatch):
    ...
    monkeypatch.setattr(
        server._codegraph_application,
        "index_and_publish",
        lambda *_args, **_kwargs: CodeGraphPublishOutcome(
            publish_mode="sqlite",
            index={"state": "ready", "revision": "sha256:local"},
        ),
    )
...
def test_wiki_code_index_refuses_hosted_source_without_delegation(monkeypatch):
    ...
    monkeypatch.setattr(
        server._codegraph_application,
        "index_and_publish",
        lambda *_args, **_kwargs: pytest.fail("hosted binding must not delegate"),
    )
...
def test_index_handler_accepts_every_known_language(seed_binding, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "index_and_publish",
        lambda binding, **values: calls.append((binding, values)) or (
            server._codegraph_application.CodeGraphPublishOutcome(
                publish_mode="sqlite",
                index={"state": "ready", "languages": values["languages"]},
            )
        ),
    )
...
def test_code_handlers_sanitize_and_log_unexpected_runtime_factory_failure(
    ...
    def fail_runtime(_source):
        raise RuntimeError(
            "secret-handler-token /absolute/private/path SELECT source SQL"
        )

    monkeypatch.setattr(server.base, "resolve_binding", lambda: seed_binding)
    monkeypatch.setattr(server._codegraph_application, "code_runtime", fail_runtime)
    monkeypatch.setattr(
        server._codegraph_application,
        "index_and_publish",
        lambda *_args, **_kwargs: fail_runtime(None),
    )
    caplog.clear()
...
def test_search_handler_logs_unexpected_failure_without_sensitive_values(
    ...
    monkeypatch.setattr(server.base, "resolve_binding", lambda: seed_binding)
    monkeypatch.setattr(
        server._codegraph_application, "code_runtime", lambda _source: runtime
    )
    monkeypatch.setattr(
        runtime_module,
        "CodeGraphQuery",
...
def test_postgres_code_store_delegates_to_application_constructor(monkeypatch):
    binding = server.base.PostgresBinding(
        host="localhost",
        port=5432,
        database="iwiki_test",
    ...
    store = object()
    calls = []
    monkeypatch.setattr(server, "_hosted_code_graph_settings", lambda: settings)
    monkeypatch.setattr(
        server._codegraph_application,
        "create_postgres_publisher",
        lambda *args, **kwargs: calls.append((args, kwargs)) or store,
    )

    assert server._postgres_code_store(binding, "owner-a") is store
    assert calls == [(
        (binding, "owner-a", settings),
        {"lock_timeout_ms": server._CODE_PUBLICATION_LOCK_TIMEOUT_MS},
    )]
