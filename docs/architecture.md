...
configured; `mcp` uses `IWIKI_CODE_GRAPH_MCP_URL` and
`IWIKI_CODE_GRAPH_MCP_TOKEN`. Configuration and invocation enforce checkout-root safety;
diagnostics redact password, token, URL, DSN, and paths in text stderr and compact
`--json`. Text and compact `--json` are output formats: either format exits `0` for a
ready snapshot, `1` for runtime/publication failure, or `2` for usage/configuration
failure. Publication activates a complete snapshot atomically, so reads see either old
or new graph, never staging rows.
...
After startup, the stdio transport keeps running for as long as its client holds stdin
open: `IWIKI_IDLE_TIMEOUT_SECONDS` defaults to `0`, no idle limit. Set a positive value
to make the process exit after that many seconds without an incoming MCP message — a
headless run that must stop by itself, not an interactive client, which already ends the
server by closing stdin. The timeout resets for every incoming message and waits for an active
...
the stdio idle shutdown and defaults to `0`, which disables it; a positive value
is for a headless run that must stop by itself.
