...
| `IWIKI_SYSTEM1_BASE_URL` | пусто | Отдельный базовый URL локального GPU-сервиса System One. Обязателен при включённом shadow; iwiki вызывает `<base>/v1/systemone`. |
