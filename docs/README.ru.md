...
| Инструмент | Что делает |
|---|---|
| `wiki_migrate_okf(domain=None)` | Добавляет фронтматтер всем страницам, у которых его нет. Два режима: **автономный** (пишет фронтматтер сразу), если задан `IWIKI_CHAT_MODEL`; иначе возвращает **план** — список кандидатов с выведенными title/description/timestamp и существующим словарём тегов домена — для классификации и применения вызывающим агентом. В автономном режиме `resource` каждой страницы при отсутствии источника берётся из последней записи лога, а теги, придуманные для одной страницы, переиспользуются как словарь для следующих страниц в этом же запуске. |
