...
| `resource` | Значение `source`, переданное инструменту записи, если оно было. |
| `tags` | Метки в нижнем регистре в формате kebab-case, не более 5 на страницу. |
| `timestamp` | Дата последнего git-коммита файла страницы, либо сегодняшняя дата, если ещё не закоммичена. |
...
Фасетный поиск сужает `wiki_search` по `type` и/или набору `tags`:
...
| Инструмент | Что делает |
|---|---|
| `wiki_migrate_okf(domain=None)` | Добавляет фронтматтер всем страницам, у которых его нет. Два режима: **автономный** (пишет фронтматтер сразу), если задан `IWIKI_CHAT_MODEL`; иначе возвращает **план** — список кандидатов с выведенными title/description/timestamp и существующим словарём тегов домена — для классификации и применения вызывающим агентом. |
| `wiki_apply_okf(domain, slug, type, tags)` | Применяет классифицированные агентом `type`/`tags` (плюс выведенные поля) как фронтматтер одной страницы, переиндексирует, коммитит и пушит. |
