...
`wiki_write_page` отказывается перезаписывать существующую страницу. У `wiki_update_page` три режима: обновление только секции требует пару `heading` и `new_body`; обновление только метаданных `code` принимает `code` и не меняет тело страницы — оно сохраняется байт в байт; совмещённое обновление атомарно выполняет оба действия. Опубликованная корневая JSON Schema использует `anyOf`, требуя `heading` + `new_body` либо `code`, а `domain` и `slug` остаются обязательными на верхнем уровне. Проверка во время выполнения отклоняет частичные, пустые или небезопасные селекторы до мутации. Непустой корректный объект `code` полностью заменяет селекторы (`code.symbols`, `code.files`, `code.source_globs`); `{}` или объект со всеми пустыми списками очищает их; пропущенный либо `null` `code` сохраняет селекторы при обновлении секции. Ответ при обновлении только `code` опускает `heading` и не добавляет полей; ответы обновления секции и совмещённого обновления сохраняют `heading`.
