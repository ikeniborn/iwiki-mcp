...
процесс (перезапуск сервера, новое stdio-подключение) вообще не возвращает ключ `job`;
опрашивающий код должен переходить на `state`/`fresh`, а не ждать вечно job, которого в
этом процессе уже нет. Внутри одного процесса ключ `job` описывает build, который идёт
сейчас, а если не идёт ни один — последний завершившийся: build всегда один, поэтому
всегда сверяйте `id` с тем дескриптором, который у вас на руках. Другой `id` означает,
что отчёт перешёл к другому build (например, к автоматическому rebuild от запроса), и
исход вашего собственного build ответ больше не описывает. Здесь тоже переходите на
`state`/`fresh`.
