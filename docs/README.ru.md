...
`read` задаёт область поиска по умолчанию для проекта. Чтобы читать из **всех** доменов базы, укажите `read = []` или вовсе уберите строку — пустой или отсутствующий `read` откатывается ко всем доменам. `read = ["all"]` **не** является подстановкой: значение трактуется как домен с буквальным именем `all`. `write` — основной целевой домен для инструментов, которым он нужен, например `wiki_index` без аргумента `domain`. `write_scope` — явный набор доменов, которые могут менять mutating-инструменты; он обязан содержать `write` и быть подмножеством `read`. Если его не указывать, сохраняется совместимость scalar `write`: writable только основной домен. `base` опционален и переопределяет `IWIKI_BASE_DIR` для этого проекта.
...
`wiki_bind` проверяет, что каждый указанный домен read и write_scope уже существует. Для существующего непустого `read` инструмент сохраняет настроенные домены и может только добавить домен текущего проекта. `write` должен совпадать с доменом текущего проекта, который берётся из имени каталога проекта. `wiki_create_domain` может bootstrap-нуть пустой отсутствующий домен вне текущего write_scope; он не создаёт страницу, индекс или лог. Привяжите домен перед первой записью в него.
...
Междоменная операция начинается, только если каждый найденный видимый referrer входит в `write_scope`; видимый read-only referrer блокирует её до изменения Markdown. Скрытые домены не инспектируются и не репортятся, поэтому никогда не переписываются. Результат дополняется полями `transaction_id`, `rewritten_pages`, `affected_domains` и `rewritten_links`.
