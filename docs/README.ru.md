...
| `wiki_update_page` | Обновить существующую страницу: только для секции (`heading` + `new_body`), только для code (`code`) или комбинированное обновление атомарно. С `new_heading` переименовать заголовок и атомарно переписать точные видимые входящие ссылки, если их домены доступны для записи. Принимает `expected_section_hash` для оптимистичной конкурентности. |
...
`wiki_write_page` отказывается перезаписывать существующую страницу. У `wiki_update_page` три режима: обновление только для секции требует парные `heading` и `new_body`; только для code использует `code` и сохраняет тело страницы byte-for-byte; комбинированное обновление атомарно выполняет оба. Опубликованная корневая JSON Schema использует `anyOf`, требуя `heading` + `new_body` либо `code`, а `domain` и `slug` остаются root-required; runtime-проверка отклоняет partial, no-op и unsafe selectors до мутации. Непустой валидный mapping `code` полностью заменяет selectors (`code.symbols`, `code.files`, `code.source_globs`); `{}` или когда все списки пусты очищает их; пропущенный либо `null` `code` сохраняет selectors при обновлении секции. Ответ только для code опускает `heading` и не добавляет полей; ответы секции и combined сохраняют `heading`.
