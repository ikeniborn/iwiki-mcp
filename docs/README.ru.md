...
ждите вечно job, которого в этом процессе уже нет. Ответ, несущий `error`, никогда не несёт `job`: ошибка описывает граф, а не
build.
...
| Инструмент | Контракт |
| --- | --- |
| `wiki_code_status` | Возвращает настройку, состояние, freshness и diagnostics локального кэша, а также дескриптор `job`, пока build выполняется или только что завершился — при любом `read_mode`. Опциональный `job_id` описывает именно этот build вместо текущего build домена; неизвестный id отвечает обычным образом с `job_unknown` в `warnings`. |
