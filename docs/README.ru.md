...
| `wiki_export_okf(domain, dest)` | Экспортирует домен в полностью OKF-совместимый пакет в `dest`: страницы сохраняют фронтматтер, синтаксис `[[wikilink]]` переписывается в обычные Markdown-ссылки, генерируются зарезервированные файлы `index.md` / `log.md`. Исходники никогда не изменяются, только копируются. |
