...
процесс открытым ровно столько, сколько позволяет транспорт. Отдельного дедлайна
публикации нет: `mcp` ограничивает каждый удалённый вызов 30 секундами на подключение и
300 секундами на чтение, а `postgres` — настроенным на сервере `statement_timeout_ms` (не
более 300 секунд), так что худший случай — эти поэтапные ограничения, умноженные на число
батчей снимка, а не бесконечное ожидание. Застрявшая публикация видна как job, который
остаётся `running` спустя долгое время после того, как фазы его build перестали
продвигаться.
