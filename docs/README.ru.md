...
| Поле | Значение |
|---|---|
| `type` | Обязательное. Закрытый словарь: `architecture`, `api`, `guide`, `reference`, `runbook`, `concept` (по умолчанию). |
| `title` | Берётся из заголовка `# Title` страницы. |
| `description` | Берётся из секции `## Overview`, обрезается до `IWIKI_SUMMARY_MAX_CHARS`. |
| `resource` | Значение `source`, переданное инструменту записи, если оно было; `wiki_apply_okf` и `wiki_migrate_okf` при отсутствии `source` берут последний зарегистрированный в логе источник страницы. |
...
| `wiki_apply_okf(domain, slug, type, tags)` | Применяет классифицированные агентом `type`/`tags` (плюс выведенные поля) как фронтматтер одной страницы, переиндексирует, коммитит и пушит. Если `tags` не переданы, существующие теги страницы сохраняются, а не очищаются. |
| `wiki_export_okf(domain, dest)` | Экспортирует домен в полностью OKF-совместимый пакет в `dest`: у каждой страницы гарантирован фронтматтер (существующие страницы сохраняют свой, теги дедуплицируются; для страниц без фронтматтера он выводится детерминированно — `type: concept`, заголовок, описание, дата коммита), синтаксис `[[wikilink]]` переписывается в обычные Markdown-ссылки, генерируются зарезервированные файлы `index.md` / `log.md`. Возвращает список `warnings`, отмечающий страницы, чьё имя совпадает с зарезервированными файлами. Исходники никогда не изменяются, только копируются. |
