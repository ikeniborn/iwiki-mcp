...
этом не удерживаются: как только граф записан, build перестаёт считаться rebuild, и
`wiki_code_status` и `wiki_code_search` всё время публикации отвечают по завершённому
локальному снимку.
