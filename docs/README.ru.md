...
`wiki_bind` проверяет, что каждый указанный домен read и write уже существует. Для существующего непустого `read` инструмент сохраняет настроенные домены и может только добавить домен текущего проекта. `primary` должен совпадать с доменом текущего проекта, который берётся из имени каталога проекта. `wiki_create_domain` может bootstrap-нуть пустой отсутствующий домен вне текущего списка write; он не создаёт страницу, индекс или лог. Привяжите домен перед первой записью в него.
...
| `wiki_update_page` | Заменить тело одной существующей секции `##`. С `new_heading` переименовать заголовок и атомарно переписать точные видимые входящие ссылки, если их домены доступны для записи. |
