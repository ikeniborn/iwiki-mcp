...
не ошибка: ответ сохраняет обычную форму и добавляет `job_unknown` в `warnings`. В обоих
случаях переходите на `state`/`fresh`, а не ждите вечно job, которого в этом процессе
уже нет. Ответ, несущий `error`, никогда не несёт `job`: ошибка описывает граф, а не
build.
