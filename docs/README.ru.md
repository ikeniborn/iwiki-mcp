...
реакцией, статусом «печатает» и статус-сообщением, которое редактируется по этапам.
Точный
...
ключуется по `mcp-session-id`, истекает после 30 минут бездействия и не переживает
