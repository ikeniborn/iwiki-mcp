...
свою длительность и останавливается по достижении потолка строк за цикл или когда
накопившееся вычерпано. Если он остановился рано — или был убит — уже закоммиченные строки остаются
...
Эту работу планирует один sweep, и никто из тех, кто его вызывает, не выполняет её сам.
`wiki_code_publish_begin`, любой другой аутентифицированный hosted-запрос и собственный
`begin` прямого CLI-публикатора в PostgreSQL (ниже) — все вызывают одну и ту же функцию
`schedule_wiki_cleanup`, которая ставит по одной работе на каждый домен из `binding.write`,
не чаще одного раза в 900 секунд на вики — это одна форма триггера, а не разное поведение
для `begin` и для всего остального. На hosted-сервере эту очередь разбирает фиксированный
набор из двух обслуживающих воркеров, поэтому рост числа клиентов даёт очередь, а не потоки
и соединения. Очередь ограничена, и постановка в переполненную отбрасывается со счётчиком —
работа вернётся со следующим запросом, потому что её никто не ждёт.
...
никогда — из того, который делят инструменты и аутентификация. Поэтому всего соединений к
PostgreSQL у сервера: пул инструментов и аутентификации (`pool_max_size` — обязательная
настройка без значения по умолчанию из коробки; 10 в примере `server.toml` в
[deployment.md](deployment.md)) плюс два обслуживающих воркера плюс, в худшем случае,
потолок инструментов (`pool_max_size - 2`, 8 при примерном размере) короткоживущих
соединений, которые три hosted-пути на запрос всё ещё открывают вне обоих пулов для
проверки принципала — 20 при примерном размере, а не двенадцать. Эти соединения
кратковременны и уже ограничены тем же потолком инструментов, что и любой другой вызов
инструмента, поэтому они не затрагивают зарезервированные соединения аутентификации; но
они означают, что `max_connections` нужно рассчитывать на эту большую сумму, а не только на
пул и воркеров.

У прямого CLI-публикатора в PostgreSQL нет hosted-сервера, поэтому нет ни пула
обслуживания, ни набора воркеров, в очередь которых мог бы встать его sweep; его
собственный вызов `begin` вместо этого переключается на один необработанный daemon-поток
на домен. CLI-путь — единственный пользователь этого fallback, а не stdio-сессия.
Этот процесс всё ещё завершается вскоре после того, как `finalize` вернул ответ, а
daemon-поток не удерживает процесс живым, поэтому запланированный им цикл очистки всё ещё
может быть убит до того, как он вычерпает накопившееся: то, что уже закоммичено, остаётся
удалённым, а остальное ждёт sweep от следующей публикации. На CLI-пути fallback
порождает один daemon-поток и одно raw-соединение на каждый writable-домен, и они
находятся вне budget соединений hosted-сервера, указанного в разделе выше, потому что они
живут в другом процессе.
