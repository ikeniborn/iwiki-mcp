...
Очистка выполняется single-flight фоновым воркером, а не внутри публикации: `begin`
планирует цикл и сразу возвращает управление, поэтому публикация никогда не
...
за цикл или когда очередь пуста. Если цикл остановился рано, следующий вызов `begin`
продолжает вычерпывание с того места, где остановился предыдущий, поскольку
...
цикл очистки, запланированный `begin`, может быть убит до того, как он вычерпает очередь.
