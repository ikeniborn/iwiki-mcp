...
закоммиченными батчами — каждый батч удаляет строки одного вытесненного снапшота, сначала
дочерние таблицы, в своей собственной транзакции — и останавливается по достижении потолка
строк за цикл или когда очередь пуста. Если цикл остановился рано, следующий продолжает
вычерпывание с того места, где остановился предыдущий, поскольку состояние хранится в базе,
а не в воркере. Одновременно на домен работает не более одного цикла: конкурентные
публикации в один и тот же домен делят его, а не складывают циклы друг на друга поверх
одних и тех же таблиц.

Эту работу планируют две вещи. `begin` планирует цикл для того домена, который публикует.
Любой другой аутентифицированный hosted-запрос может вместо этого запланировать *свёртку*
(sweep): по одному циклу на каждый домен из `binding.write`, по одному домену за раз, не
чаще одного раза в 900 секунд на вики; вызов, попавший в неистёкший интервал, платит только
за взятие лока и сравнение float. Свёртка появилась потому, что одной публикации было
недостаточно: домен, переставший публиковаться, держал свои вытесненные строки вечно. Её
охват — это `binding.write` вызывающего, то есть домены, в которые он и так может писать, и
каждый домен чистит свой собственный store, построенный и проверенный ровно под этот домен.
Этот же мандат объясняет, почему триггером выбран запрос, а не таймер: у таймера нет
привязки, и ему пришлось бы действовать всем охватом сервисной роли. Поэтому вики, к которой
никто не обращается, никогда не сворачивается.
