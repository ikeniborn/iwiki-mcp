...
Очистка выполняется single-flight фоновым воркером, а не внутри публикации: `begin` и
`finalize` планируют цикл и сразу возвращают управление, поэтому публикация никогда не
задерживается и не падает из-за неё. Цикл вычерпывает накопившееся закоммиченными
батчами — каждый батч удаляет строки одного вытесненного снапшота, сначала дочерние
таблицы, в своей собственной транзакции — и останавливается по достижении потолка строк
за цикл или когда очередь пуста. Если цикл остановился рано, следующий вызов планирования
публикацией продолжает вычерпывание с того места, где остановился предыдущий, поскольку
состояние хранится в базе, а не в воркере. Одновременно на домен работает не более одного
цикла: конкурентные публикации в один и тот же домен делят его, а не складывают циклы друг
на друга поверх одних и тех же таблиц. Прямая публикация через CLI в PostgreSQL — известный
пробел здесь: процесс завершается вскоре после того, как `finalize` вернул ответ, поэтому
запланированный им цикл очистки может быть убит до того, как он вычерпает очередь.
