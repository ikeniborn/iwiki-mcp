...
def children_running(required_children, run=subprocess.run):
    try:
        result = run(
...
def local_http_ok(host, port, path, connection_factory=http.client.HTTPConnection):
    connection = None
    healthy = False
    try:
        connection = connection_factory(host, port)
        connection.request("GET", path)
        healthy = connection.getresponse().status in {401, 405}
    except Exception:
        healthy = False
    finally:
...
def main(
    *,
    ...
    if not children_running(REQUIRED_CHILDREN, run):
        return fail("child_not_running")
    if not local_http_ok("127.0.0.1", 8765, "/mcp", connection_factory):
